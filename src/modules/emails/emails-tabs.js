...
export function initializeEmailTabs() {
    ...
    // Initialize expand/collapse toggle for email logs
    const toggleBtn = document.querySelector('.email-logs-toggle');
    if (toggleBtn) {
        toggleBtn.addEventListener('click', () => {
            const logsBody = document.querySelector('.email-logs-body');
            if (logsBody) {
                const isExpanded = logsBody.classList.toggle('expanded');
